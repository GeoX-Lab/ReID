...
from os_modules import OSBlock
...
class RGA_Branch(nn.Module):
	def __init__(self, pretrained=True, last_stride=1, block=Bottleneck, layers=[3, 4, 6, 3],
		...
		print('Use_Spatial_Att: {};\tUse_Channel_Att: {};\tUse_O_Scale: {}.'.format(spa_on, cha_on, use_o_scale))
		...
		self.layer1 = self._make_rga_layer(block, 64, layers[0])
		self.layer2 = self._make_rga_layer(block, 128, layers[1], stride=2)
		self.layer3 = self._make_rga_layer(block, 256, layers[2], stride=2)
		self.layer4 = self._make_rga_layer(block, 512, layers[3], stride=last_stride)
		...
		# OS Modules
		if use_o_scale:
			self.o_scale1 = OSBlock(in_channels=1024, out_channels=1024, IN=True)
			self.o_scale2 = OSBlock(in_channels=1024, out_channels=2048, IN=True)
	...
	def _make_rga_layer(self, block, channels, blocks, stride=1):
		downsample = None
	...
	def forward(self, x):
		...
		x = self.layer3(x)
		if self.use_o_scale:
			y = self.o_scale1(x)  # insert the os module
			y = self.o_scale2(y)
		x = self.rga_att3(x)  # the rga module
